...
for f in xui-sync.sh xui-panel-relay.sh xui-free-config-probe.sh panel-relay-lib.sh free-config-probe-lib.sh panel-relay-exec.php hooshpay-relay-lib.sh hooshpay-relay-exec.php ai-relay-lib.sh ai-relay-exec.php install-ollama.sh install-persian-ai.sh free-config-probe-exec.php free-config-probe-bootstrap.php install-xray.sh enable-free-config-probe.sh xui-services.sh install-vps.sh update-vps.sh config.example.sh; do
    if [ -f "$src/$f" ]; then
