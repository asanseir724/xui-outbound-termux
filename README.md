...
نصب‌کننده سرویس‌ها را با **runit (termux-services)** بالا می‌آورد و `termux-wake-lock` را روشن می‌کند تا با بستن پنجره‌ی Termux خاموش نشوند. برای پایداری کامل بعد از ریبوت:
...
| توقف سرویس کرون | `sv down crond` |
| دیدن کرون فعلی | `crontab -l` |
| باز کردن پنل وب | مرورگر → `http://localhost:8088` |
| ری‌استارت پنل وب | `sv restart xui-panel` |
